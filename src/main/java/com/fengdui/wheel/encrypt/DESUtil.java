...
public class DESUtil {
	...
	private static Map<String, DESUtil> desMap = new HashMap<String, DESUtil>();

	/** 字符串默认键值 */
	private static String defaultKeyStr = "national";
	/** 加密工具 */
	private Cipher encryptCipher = null;
	/** 解密工具 */
	private Cipher decryptCipher = null;

	public static DESUtil getInstance(String keyStr) throws Exception {
		if (StringUtils.isBlank(keyStr)) {
			keyStr = defaultKeyStr;
		}
		DESUtil instance = desMap.get(keyStr);
		if (null == instance) {
			instance = new DESUtil(keyStr);
			desMap.put(keyStr, instance);
		}
		return instance;
	}
	...
	/**
	 * 将byte数组转换为表示16进制值的字符串<br>
	 * 如：byte[]{8,18}转换为：0813,和public static byte[] hexStr2ByteArr(String strIn) 互为可逆的转换过程
	...
	public static void main(String[] args) {
		try {
			...
			DESUtil des2 = new DESUtil("wander");// 自定义密钥  
