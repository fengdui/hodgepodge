package com.fengdui.framework.file;
...
public class FileUtils {
    ...
    public static void copy(InputStream inputStream, File target) {
        ...
        try {
            ...
            logger.error("Error while copying an input stream to file '"
    ...
    public static void copy(File source, OutputStream outputStream) {
        ...
        try {
            ...
            logger.error("Error while copying file '"
                    + source.getAbsolutePath() + "' to an output stream.");
        }
    }
        ...
        if (destPath != null) {
            try {
                ...
                FileOutputStream file_output_stream = new FileOutputStream(
                        destDir.getPath() + File.separator + source.getName());
                ...
                logger.error("Error while copying file '"
                        + source.getAbsolutePath() + "' to '" + destPath + "'.");
            }
        }
    }
     ...
     * @throws IOException 读取或创建新文件有误时抛出
     */
    ...
    public static String toLocalFilePath(String URLPath) {
        ...
        if (File.separatorChar == '\\')
            ...
            logger.error("Not Support '" + File.separatorChar
                    + "' to be File.separatorChar");
            return null;
        }
    }
    ...
    public static void unZip(String fileName, String filePath)
        ...
        try {
            ...
            e.printStackTrace();
            logger.error("Error while unZip file '" + fileName + "' to '"
                    + filePath + "'.");
        }
    ...
    public static String getMD5(File file) {
        try {
            ...
            in.close();
            return toHex(md5.digest()).toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * 显示文件的容量大小
     */
    public static String getShowFilesize(Integer fileSize) {
        if (null == fileSize) {
            return "0B";
        }
        DecimalFormat format = null;
        if (fileSize > 1024 * 1024) {
            format = new DecimalFormat("0.00M");
            return format.format((fileSize) / 1024f / 1024f);
            //return String.format("%3.1fMB", ((float) fileSize) / 1024 / 1024);
        } else if (fileSize > 1024) {
            format = new DecimalFormat("0K");
            return format.format(fileSize / 1024f);
            //return String.format("%dKB", fileSize / 1024);
        } else {
            format = new DecimalFormat("0B");
            return format.format(fileSize);
    ...
    public static String getShowFilesize(Long fileSize) {
        ...
        return getShowFilesize(fileSize.intValue());
    }
    ...
    public static byte[] getFileData(String fileUrl) {
        ...
        try {
            ...
            for (int len = 0; -1 != (len = inputStream.read(buf)); ) {
                outputStream.write(buf, 0, len);
            }
            return outputStream.toByteArray();
        } catch (Exception e) {
            logger.error("getFileData " + fileUrl + " error", e);
            return null;
        } finally {
            try {
                if (null != connection) {
    ...
    public static InputStream getFileInputStream(String fileUrl) {
        ...
        try {
            ...
            logger.error("getFileInputStream " + fileUrl + " error", e);
            return null;
        } finally {
            try {
                if (null != connection) {
